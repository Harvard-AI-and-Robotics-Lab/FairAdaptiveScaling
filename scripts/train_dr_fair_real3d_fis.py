...
def train(model, criterion, optimizer, scaler, train_dataset_loader, epoch, total_iteration, dataset_info=None, num_classes=2, attr_index=0,  loss_scaler=None, args=None):
    global device
...
#             model = ConvNet_3D(out_dim=out_dim)
    ...
    attribute_type_map_to_index = {'race': 0, 'gender': 1, 'hispanic': 2, 'maritalstatus': 3, 'language': 4, 'genderrace': 5, 'genderethnicity': 6}
    attribute_type_map_to_num = {'race': 3, 'gender': 2, 'hispanic': 2, 'maritalstatus': 5, 'language': 3, 'genderrace': 5, 'genderethnicity': 4}
    ...
    for epoch in range(start_epoch, args.epochs):
        train_loss, train_acc, train_auc, train_preds, train_gts, train_attrs = train(model, criterion, optimizer, scaler, train_dataset_loader, epoch, total_iteration, dataset_info=ds_info, num_classes=args.num_classes, attr_index=attribute_index, loss_scaler=loss_scaler, args=args)
