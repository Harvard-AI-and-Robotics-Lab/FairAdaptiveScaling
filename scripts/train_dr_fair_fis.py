...
import models_vit
from models_vit import NativeScalerWithGradNormCount, adjust_learning_rate, param_groups_lrd, param_groups_lrd_fis, interpolate_pos_embed
...
#         model = OphBackbone(model_type=args.model_type, in_dim=in_dim, coef=args.fuse_coef)
    ...
    if args.model_type == 'ViT-B':
        ...
        param_groups = param_groups_lrd_fis(model, loss_scaler, args.weight_decay,
            no_weight_decay_list=no_weight_decay_list,
            layer_decay=args.layer_decay
        )
#         print(loss_scaler.named_parameters())
...
#         model = create_model(model_type=args.model_type, in_dim=in_dim, out_dim=out_dim)
    ...
    for epoch in range(start_epoch, args.epochs):
        train_loss, train_acc, train_auc, trn_preds, trn_gts, trn_attrs = train(model, criterion, optimizer, scaler, train_dataset_loader, epoch, total_iteration, dataset_info=ds_info, num_classes=args.num_classes, attr_index=attribute_index, loss_scaler=loss_scaler, args=args)
