...
from src.acsconv.converters import ACSConverter, Conv3dConverter, Conv2_5dConverter, SoftACSConverter
...
def train(model, criterion, optimizer, scaler, adv_model, adv_criterion, adv_optimizer, train_dataset_loader, epoch, total_iteration, dataset_info=None, num_classes=2, args=None):
    global device
    ...
    for i, (input, target, attr) in enumerate(train_dataset_loader):
        ...
        with torch.cuda.amp.autocast():
            ...
            pred = model(input) # .squeeze(1)   # input: (b, 1, 128, 200, 200) ---> pred: (b, 1)
            ...
            p_z = adv_model(torch.unsqueeze(pred, 1))
#             pred = pred.squeeze(1)
            
            if args.attribute_type=='race':
                one_hot_attr = F.one_hot(attr[0].long(), num_classes=3).float().to(device)
                loss_adv = adv_criterion(p_z, one_hot_attr) * lambdas
            elif args.attribute_type=='gender':
                loss_adv = adv_criterion(p_z.squeeze(), attr[1].float().to(device)) * lambdas
            elif args.attribute_type=='ethnicity':
                loss_adv = adv_criterion(p_z.squeeze(), attr[2].float().to(device)) * lambdas    
            elif args.attribute_type=='maritalstatus':
                one_hot_attr = F.one_hot(attr[3].long(), num_classes=5).float().to(device)
                loss_adv = adv_criterion(p_z, one_hot_attr) * lambdas
            elif args.attribute_type=='language':
                one_hot_attr = F.one_hot(attr[4].long(), num_classes=3).float().to(device)
                loss_adv = adv_criterion(p_z, one_hot_attr) * lambdas
            elif args.attribute_type=='gender_race':
                one_hot_attr = F.one_hot(attr[5].long(), num_classes=6).float().to(device)
                loss_adv = adv_criterion(p_z, one_hot_attr) * lambdas
            elif args.attribute_type=='gender_ethnicity':
                one_hot_attr = F.one_hot(attr[6].long(), num_classes=4).float().to(device)
                loss_adv = adv_criterion(p_z, one_hot_attr) * lambdas
        ...
        print(pred.shape[1], loss, loss_adv)
...
if __name__ == '__main__':
    ...
    train_havo_dataset = Harvard_DR_Fairness(os.path.join(args.data_dir, 'train'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type, needBalance=args.need_balance, transform=train_transform, dataset_proportion=args.dataset_proportion)
    val_havo_dataset = Harvard_DR_Fairness(os.path.join(args.data_dir, 'val'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, transform=eval_transform, attribute_type=args.attribute_type)
    test_havo_dataset = Harvard_DR_Fairness(os.path.join(args.data_dir, 'test'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, transform=eval_transform, attribute_type=args.attribute_type)
...
#             model = ConvNet_3D(out_dim=out_dim)
            ...
            # by Hao ---------------
            if args.conv_type == 'Soft_ACSConv':
                model = model_to_syncbn(SoftACSConverter(model))
                print('using SoftACSConv model')
            # ----------------------
    ...
    if args.attribute_type=='race' or args.attribute_type=='language':
        adv_model = Adversary(n_sensitive=3).to(device)
        adv_criterion = torch.nn.CrossEntropyLoss()
    elif args.attribute_type=='maritalstatus':
        adv_model = Adversary(n_sensitive=5).to(device)
        adv_criterion = torch.nn.CrossEntropyLoss()
    elif args.attribute_type=='gender_race':
        adv_model = Adversary(n_sensitive=6).to(device)
        adv_criterion = torch.nn.CrossEntropyLoss()
    elif args.attribute_type=='gender_ethnicity':
        adv_model = Adversary(n_sensitive=4).to(device)
        adv_criterion = torch.nn.CrossEntropyLoss()
    else:
...
#         model = nn.DataParallel(model)
    ...
    best_test_auc = sys.float_info.min

    for epoch in range(start_epoch, args.epochs):
        train_loss, train_acc, train_auc, train_preds, train_gts, train_attrs = train(model, criterion, optimizer, scaler, adv_model, adv_criterion, adv_optimizer, train_dataset_loader, epoch, total_iteration, dataset_info=ds_info, num_classes=args.num_classes, args=args)
        ...
        logger.log(f'---- best test AUC {best_test_auc:.4f} at epoch {best_ep}')
